...
export default function KegiatanSection() {
  ...
  /* ================= RENDER ================= */
  return (
    <section id="Kegiatan" className="py-16 bg-gradient-to-b from-blue-50 to-white">
...
/* ================= SUB COMPONENT ================= */
function Card({ item, mobile }) {
  return (
    <div className={`overflow-hidden rounded-3xl bg-white shadow-lg ${mobile ? 'max-w-full' : ''}`}>
      {/* Image */}
      <img
        src={item.image}
        alt={item.title}
      ...
      <div className={`${mobile ? 'p-8' : 'p-6'}`}>
        <h3 className={`font-bold mb-2 text-gray-800 ${mobile ? 'text-2xl' : 'text-xl'}`}>
          {item.title}
        </h3>
        <p className={`text-gray-500 mb-3 ${mobile ? 'text-base' : 'text-sm'}`}>
          {item.date}
        </p>
        <p className={`text-gray-600 line-clamp-3 ${mobile ? 'text-base leading-relaxed' : 'text-sm'}`}>
          {item.shortDesc}
        </p>
      </div>
    </div>
